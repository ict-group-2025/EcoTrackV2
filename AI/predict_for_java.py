# Tên file: predict_for_java.py - Phiên bản tích hợp với Java Spring
import joblib
import numpy as np
import pandas as pd
import json
...
def run_forecast_for_city(city="Hanoi"):
    """
    Chạy dự báo cho một thành phố cụ thể và trả về JSON
    """
    try:
        print(f"Đang tải các mô hình AI từ ổ cứng...")
        lstm_model = load_model('lstm_hourly_model.keras')
        scaler = joblib.load('scaler_hourly.pkl')
        
        print(f"Đang lấy dữ liệu 24h gần nhất từ OpenWeather API cho {city}...")
        df_api_hourly = get_openweather_hourly_data(city)
        ...
        print(f"Đang dự báo thời tiết cho 24 giờ tiếp theo cho {city}...")
        ...
        # Tạo danh sách mốc thời gian cho 24 giờ tới
        now = datetime.now()
        current_hour_rounded = now.replace(minute=0, second=0, microsecond=0)
        
        # Tạo kết quả JSON cho Java
        forecasts = []
        for i in range(1, 25):
            ...
            forecast = {
                "hour_offset": i,
                "forecast_time": next_hour.isoformat(),
                "temperature": round(float(predicted_24h_temp[i-1]), 2),
                "model_type": "LSTM",
                "confidence": 0.85  # Confidence score cố định, có thể tính toán sau
            }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='AI Weather Forecast for Java Spring Integration')
    parser.add_argument('--city', type=str, default='Hanoi', help='City name for forecast')
    parser.add_argument('--original', action='store_true', help='Run original forecast function')
    
    args = parser.parse_args()
    
    if args.original:
        run_forecast_original()
    else:
        run_forecast_for_city(args.city)
